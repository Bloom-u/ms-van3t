...
namespace ns3
{
  NS_LOG_COMPONENT_DEFINE("stopWarningServer");

  NS_OBJECT_ENSURE_REGISTERED(stopWarningServer);

  TypeId
  stopWarningServer::GetTypeId (void)
  {
    static TypeId tid =
        TypeId ("ns3::stopWarningServer")
        .SetParent<Application> ()
        .SetGroupName ("Applications")
        .AddConstructor<stopWarningServer> ()
        .AddAttribute ("AggregateOutput",
           "If it is true, the server will print every second an aggregate output about cam and denm",
           BooleanValue (false),
           MakeBooleanAccessor (&stopWarningServer::m_aggregate_output),
           MakeBooleanChecker ())
        .AddAttribute ("RealTime",
           "To compute properly timestamps",
           BooleanValue(false),
           MakeBooleanAccessor (&stopWarningServer::m_real_time),
           MakeBooleanChecker ())
        .AddAttribute ("CSV",
            "CSV log name",
            StringValue (),
            MakeStringAccessor (&stopWarningServer::m_csv_name),
            MakeStringChecker ())
        .AddAttribute ("Client",
           "TraCI client for SUMO",
           PointerValue (0),
           MakePointerAccessor (&stopWarningServer::m_client),
           MakePointerChecker<TraciClient> ())
        .AddAttribute ("PRRSupervisor",
            "PRR Supervisor to compute PRR according to 3GPP TR36.885 V14.0.0 page 70",
            PointerValue (0),
            MakePointerAccessor (&stopWarningServer::m_PRR_supervisor),
            MakePointerChecker<PRRSupervisor> ());
        return tid;
  }

  stopWarningServer::stopWarningServer ()
  {
  ...
  void
  stopWarningServer::DoDispose (void)
  {
    NS_LOG_FUNCTION(this);
  ...
  void
  stopWarningServer::StartApplication (void)
  {
    NS_LOG_FUNCTION(this);
    ...
    */
    ...
    m_denService.addDENRxCallback (std::bind(&stopWarningServer::receiveDENM,this,std::placeholders::_1,std::placeholders::_2));
    ...
    m_caService.addCARxCallback (std::bind(&stopWarningServer::receiveCAM,this,std::placeholders::_1,std::placeholders::_2));
    ...
    if (m_aggregate_output)
      m_aggegateOutputEvent = Simulator::Schedule (Seconds(1), &stopWarningServer::aggregateOutput, this);
  }

  void
  stopWarningServer::StopApplication ()
  {
    NS_LOG_FUNCTION(this);
  ...
  void
  stopWarningServer::StopApplicationNow ()
  {
    NS_LOG_FUNCTION(this);
    StopApplication ();
  }
...
//    /* Build DENM data */
   ...
   // RoadWorksContainerExtended, inside the "A la carte" container, which actually has a "SpeedLimit" field
    ...
    m_update_denm_ev = Simulator::Schedule (Seconds (1), &stopWarningServer::UpdateDenm, this);
  }

  void
  stopWarningServer::UpdateDenm()
  {
    denData data;
    DEN_RoadWorksContainerExtended_t roadworks;
    denData::denDataAlacarte alacartedata;
  ...
  void
  stopWarningServer::TerminateDenmTransmission()
  {
    m_isTransmittingDENM=false;
    Simulator::Cancel (m_update_denm_ev);
  }
  ...
  void
  stopWarningServer::receiveCAM (asn1cpp::Seq<CAM> cam, Address from)
  {
    ...
    std::cout << "RSU Received a new CAM from" << from << std::endl;
    //获取rsu的位置
    Ptr<MobilityModel> mob = GetNode ()->GetObject<MobilityModel>();
    std::cout << "rsu position: "<< mob->GetPosition ().x<<" "<<mob->GetPosition ().y<<std::endl;
    for (int i = 0; i < 50; i++) {
      std::cout << "-";
    }
    std::cout << std::endl;
    ...
    if(m_isTransmittingDENM)
    {
      ...
      Simulator::Cancel (m_terminate_denm_ev);
      m_terminate_denm_ev = Simulator::Schedule (Seconds (5), &stopWarningServer::TerminateDenmTransmission, this);
    }
    else
    {
...
//    ASN_STRUCT_FREE(asn_DEF_CAM,cam);
  ...
  void
  stopWarningServer::receiveDENM (denData denm, Address from)
  {
  ...
  long
  stopWarningServer::compute_timestampIts ()
  {
  ...
  void
  stopWarningServer::aggregateOutput()
  {
    std::cout << Simulator::Now () << "," << m_cam_received << "," << m_denm_sent << std::endl;
    m_aggegateOutputEvent = Simulator::Schedule (Seconds(1), &stopWarningServer::aggregateOutput, this);
  }
